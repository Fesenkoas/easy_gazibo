      ...
      if (item) {
        ...
        if (isFabric) {
          ...
          if (isDate < 0) {
            ...
            if (isName < 0) {
              await PrintFile.findByIdAndUpdate(
                isFabric._id,
                { $push: { "folderDate.$[date].item": { fullUrl: url, fileName: name, height, width, col, print: false, stop: false, waste: 0 } } },
          ...
          const newItem = new PrintFile({
            folderFabric,
            folderDate: [{ date: folderDate, item: [{ fullUrl: url, fileName: name, height, width, col, print: false, stop: false, waste: 0 }] }],
          });
          await newItem.save();
